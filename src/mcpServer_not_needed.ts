...
const server = new McpServer({
  name: 'understand-aws-spend-mcp-server',
  version: '1.0.0',
...
server.tool(
  ...
  async ({ name }, { sendNotification }): Promise<CallToolResult> => {
    ...
    await sendNotification({
      ...
      params: { level: "error", data: `Starting multi-greet for ${name}` }
    });
    ...
    await sendNotification({
      method: "notifications/message",
      params: { level: "error", data: `Sending first greeting to ${name}` }
    });
    ...
    await sendNotification({
      method: "notifications/message",
      params: { level: "error", data: `Sending second greeting to ${name}` }
    });
